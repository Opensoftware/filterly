...
RSpec.describe Filterly::Tree do
  ...
  describe '#prepend_ast' do
    it 'throws exception when trying to prepend to not root of the tree' do
      object = described_class.new(new_node)

      expect { object.prepend_ast(new_node, :or) }.to raise_error('Not a tree root!')
    end

    it 'adds a new node to root of the tree and returns root' do
      expect(subject.to_ast).to eql(ast)

      result = subject.prepend_ast(new_node, :or)
